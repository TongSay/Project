...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        <div>
            <h4 class="mb-3 mb-md-0">{{ __('globle.Welcome_to') }} {{ __('globle.Loans') }}</h4>
        </div>
                ...
                <div class="align-self-center">
                    ...
                    @if (@isset(auth()->user()->role->permission['name']['loan']['can-add']))
                        ...
                        <a href="{{ route('loans.create') }}">
                            ...
                            <button type="button" class="btn btn-primary btn-icon-text mb-2 mb-md-0">
                                <i class="btn-icon-prepend" data-feather="plus-square"></i> {{ __('globle.Loans') }}
                            </button>
                            ...
                            <table id="dataTableExample" class="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="py-6">{{ __('globle.Types') }} {{ __('globle.Loans') }}</th>
                                ...
                                <tbody>
                                    @if (count($loans) > 0)
                                        @foreach ($loans as $key => $loan)
                                            ...
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>{{ $loan->{'title_'.app()->getLocale()} }}</td>
                                                <td>{{ $loan->loan->{'title_'.app()->getLocale()} }}</td>
                                                
                                                <td>{{ Str::limit($loan->{'description_'.app()->getLocale()}, 70) }}</td>
                                                <td>

                                                    @if (@isset(auth()->user()->role->permission['name']['loan']['can-edit']))
                                                    ...
                                                    @if (@isset(auth()->user()->role->permission['name']['loan']['can-delete']))
                                                        <button type="button" class="btn btn-link" data-toggle="modal"
                                                            data-target="#exampleModal{{ $loan->id }}">
