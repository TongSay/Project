...
@section('content')
    ...
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('loans.index') }}">{{ __('globle.Action.Back') }}</a></li>
            <li class="breadcrumb-item active" aria-current="page">{{ __('globle.Action.Edit') }} {{ __('globle.Loans') }}
            </li>
        </ol>
    </nav>
    ...
    <div class="row">
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">{{ __('globle.Action.Edit') }} {{ __('globle.Loans') }}</li>
                        </ol>
                    </h4>

                    <form class="cmxform" action="{{ route('loans.update', [$loan->id]) }}" method="POST"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        <label for="id_loan_type">{{ __('globle.Action.Select') }} {{ __('globle.Types') }}
                                            {{ __('globle.Loans') }}</label>

                                        <select name="id_loan_type" id="id_loan_type"
                                            class="form-control @error('id_loan_type') is-invalid @enderror" required="">
                                            <option value="">{{ __('globle.Action.Select') }} </option>
                                            @foreach (App\Models\LoanType::all() as $loantype)

                                                <option value="{{ $loantype->id }}" @if ($loan->id_loan_type == $loantype->id)
                                                    Selected
                                            @endif>
                                    ...
                                    @foreach (config('app.available_locales') as $locale)
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" type="text" name="title_{{ $locale }}"
                                                id="title_{{ $locale }}" value="{{ $loan->{'title_' . $locale} }}"
                                                required autofocus />
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control" name="description_{{ $locale }}"
                                                ...
                                                rows="5">{{ $loan->{'description_' . $locale} }}</textarea>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea cols="10" id="editor1_{{ $locale }}"
                                                ...
                                                required="">{{ $loan->{'content_' . $locale} }}</textarea>
                                            @error('content')
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <center>
                                            ...
                                            <img src="{{ asset('LoanPicture') }}/{{ $loan->image }}" alt="Loan Picture "
                                                id="output" style="width: 100%; border-radius:5px;" />

                                            <br>

                                            <input type="file" hidden accept="image/*" name="image" id="file"
                                                onchange="loadFile(event)" style="display: none;">
                                            <label class="btn btn-primary mt-3" for="file" style="cursor: pointer;">Upload
                                                Image</label>
